...
If the MethodsNET website changes, students should inspect the saved raw HTML
...
def write_table(path: str | Path, rows: list[dict[str, Any]], fieldnames: list[str]) -> Path:
    ...
    Stable headers are useful for teaching because students can open the file
...
def text_after_heading(soup: BeautifulSoup, heading_pattern: str, *, max_parts: int = 6) -> str | None:
    ...
    rule for all websites. The raw HTML is saved so students can revise this
...
def main() -> None:
    ...
    # the course-list page. Students often expect "all" to be possible, so the
    ...
    for row in course_rows_for_details[:detail_limit]:
        ...
        # Add listing guesses so students can compare list-page and detail-page
